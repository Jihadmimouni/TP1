...
func NewSimulator() *Simulator {
	return &Simulator{
		...
		logger:         NewLogger(),
		SnapCompleted:  make(map[string]bool),
	}
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	for _, serverId := range getSortedKeys(sim.servers) {
		sim.SnapCompleted[serverId] = false
	}
}
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// TODO: IMPLEMENT ME
	sim.SnapCompleted[serverId] = true
	completed := true
	for _, Id := range getSortedKeys(sim.SnapCompleted) {
		if !sim.SnapCompleted[Id] {
			completed = false
		}
	}
	if completed {
